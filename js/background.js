      ...
      general: {
        ...
        engine: {
          ...
          imgengine: [
            {
              name: "Google Images",
              content: "https://lens.google.com/uploadbyurl?url=%s",
            },
            {
              name: "Bing Images",
              content: "https://www.bing.com/images/search?q=imgurl:%s",
            },
          ],
        },
        script: {
...
var appConfmodel = {
  rss: {
    feed: ["https://www.baidu.com/s?wd=1"],
    n_optype: "s_back",
  ...
  extmgm: {
    ...
    always:
      [] /*,group:[chrome.i18n.getMessage("extmgm_gplast"),chrome.i18n.getMessage("extmgm_gpalways")],exts:[]*/,
  },
  notepad: {
...
var sub = {
  ...
  cons: {
    ...
    per_write: false,
    scroll: {},
    crversion: navigator.userAgent
      .substr(navigator.userAgent.indexOf("Chrome") + 7, 100)
      .substr(
        0,
        navigator.userAgent
          .substr(navigator.userAgent.indexOf("Chrome") + 7, 100)
    ...
    if (
      ...
      if (item == null) {
        sub.checkPermission(
          ...
          null,
          null,
          sub.getI18n("perdes_browsersettings")
        );
  ...
  initEvent: function () {
    ...
    for (
      ...
      if (
        config.mges.actions[i].name == "pretab" &&
  ...
  getConfValue: function (type, value) {
    ...
    for (
      var i = 0;
      i < (sub.theConf[type] ? sub.theConf[type].length : 0);
      i++
    ) {
  ...
  getId: function (value) {
    ...
    switch (value) {
      ...
      case "s_left":
        ...
        for (
          var i = 0;
          i < sub.curWin.tabs.length && sub.curWin.tabs.length > 1;
          i++
        ) {
  ...
  getIndex: function (value, type) {
    ...
    switch (value) {
      ...
      case "s_right":
        if (_new) {
          theIndex.push(
            sub.curTab.index == sub.curWin.tabs.length - 1
              ? sub.curWin.tabs.length
              : sub.curTab.index + 1
          );
          break;
        }
  ...
  checkPermission: function (thepers, theorgs, theFunction, msg) {
    ...
    if (thepers && theorgs) {
      chrome.permissions.contains(
        { permissions: thepers, origins: theorgs },
        function (result) {
          checkPers(result);
        }
      );
    } else if (thepers) {
      chrome.permissions.contains({ permissions: thepers }, function (result) {
        checkPers(result);
      });
    } else if (theorgs) {
      chrome.permissions.contains({ origins: theorgs }, function (result) {
        checkPers(result);
      });
    }
  ...
  action: {
    ...
    back: function () {
      //chk
      // chrome.scripting.executeScript(
      //   { code: "window.history.go(-1)", runAt: "document_start" },
      //   function () {}
      // );
      // chrome.scripting.executeScript(
      //   {
      //     target: { tabId: chrome.tabs.TAB_ID_NONE }, // 你需要指定一个有效的 tabId
    ...
    scroll: function () {
      ...
      // fetch(chrome.runtime.getURL('js/inject/scroll.js'))
    ...
    next: function () {
      if (sub.message.npok) {
        ...
        sub.cons.next = {};
        sub.cons.next.index = sub.getIndex(
    ...
    pretab: function () {
      ...
      for (var i = 1; i < sub.cons.pretab.length; i++) {
        if (
          sub.cons.pretab[i].tabId != curTabId &&
          ids.contains(sub.cons.pretab[i].tabId)
        ) {
          console.log(sub.cons.pretab[i].tabId);
          chrome.tabs.update(
            sub.cons.pretab[i].tabId,
            { active: true },
            function () {
    ...
    close: function () {
      var ids = sub.getId(
          sub.getConfValue("selects", "n_tab"),
      ...
      let funClose = function () {
        let _funClose = function () {
          if (
            sub.curWin.tabs.length == ids.length &&
            !sub.curWin.incognito &&
            _closeKeep
          ) {
      ...
      let setNum = function (txt) {
        ...
        if (_num == "") {
          ...
          if (_numNew.toString().length < _num.length) {
            _numNew =
              "0".repeat(_num.length - _numNew.toString().length) + _numNew;
          }
      ...
      chrome.windows.create(
        {
          ...
          incognito: true,
          state:
            sub.getConfValue("selects", "n_optype") == "s_incog"
              ? "normal"
              : "minimized",
        },
        function (window) {
    ...
    switchtab: function () {
      ...
      if (
        (sub.message.type == "action_wges" ||
          sub.message.type == "action_rges") &&
        sub.message.sendValue.buttons == 2
      ) {
        chrome.tabs.sendMessage(
          id,
          { type: "fix_switchtab", value: "contextmenu" },
          function (response) {
    ...
    move: function () {
      var index = sub.getIndex(
        sub.getConfValue("selects", "n_position_lrhl")
      )[0];
    ...
    pin: function () {
      ...
      if (ids.length == 1) {
        ...
        chrome.windows.getCurrent({ populate: true }, function (window) {
          for (
            var i = 0;
            i < window.tabs.length /*&&ids.contains(window.tabs[i].id)*/;
            i++
          ) {
    ...
    copytabele: function () {
      var theFunction = function () {
        var theIndex = sub.getIndex(
          sub.getConfValue("selects", "n_tab_single")
        )[0];
        chrome.tabs.query(
          { index: theIndex, currentWindow: true },
          function (tabs) {
            var cptarget = tabs[0];
      ...
      chrome.windows.getCurrent(function (window) {
        chrome.windows.getAll(
          ...
          function (windows) {
            ...
            sub.cons.mergewin.lastwins =
              win.length > 0 ? win : sub.cons.mergewin.lastwins;
                      ...
                      if (sub.cons.mergewin.lastwins[_temp].length > 1) {
                        ...
                        chrome.tabs.move(
                          ...
                          function () {
                            if (
                              _temp ==
                              sub.cons.mergewin.lastwins.length - 1
                            ) {
    ...
    tts: function () {
      ...
      var theFunction = function () {
        ...
        (_rate = sub.getConfValue("ranges", "n_rate")
          ? Number(sub.getConfValue("ranges", "n_rate"))
          : 1),
          ...
          if (!id) {
            _notif
              ? sub.showNotif(
                  "basic",
                  sub.getI18n("notif_title"),
                  sub.getI18n("notif_con_dnimgerr")
                )
              : null;
          } else {
            _notif
      ...
      if (!chrome.tabs.setZoom) {
        ...
        var factorCus = _confData.value
            ? _confData.valueOption == "cl_factorcustom"
              ? parseInt(_confData.valueSetting) / 100
              : false
            : false,
            ...
            chrome.tabs.getZoom(function (zoom) {
              ...
              for (var i = zoomRange.length - 1; i >= 0; i--) {
                if (zoom >= parseFloat((zoomRange[i] / 100).toFixed(2))) {
                  zoom = parseFloat(
                    (
                      zoomRange[i == zoomRange.length - 1 ? 0 : i + 1] / 100
                    ).toFixed(2)
                  );
          ...
          case "s_out":
            chrome.tabs.getZoom(function (zoom) {
              ...
              for (var i = 0; i < zoomRange.length; i++) {
                if (zoom < parseFloat((zoomRange[i] / 100).toFixed(2))) {
                  zoom = parseFloat(
                    (zoomRange[i == 0 ? 0 : i - 2] / 100).toFixed(2)
                  );
    ...
    zoom_dep: function () {
      for (
        var i = 0;
        i < (sub.theConf.selects ? sub.theConf.selects.length : 0);
        i++
      ) {
    ...
    mute: function () {
      ...
      if (_mute == "s_audible") {
        chrome.windows.getAll({ populate: true }, function (windows) {
          for (var i = 0; i < windows.length; i++) {
            chrome.tabs.query(
              { audible: true, windowId: windows[i].id },
              function (tabs) {
                for (var i = 0; i < tabs.length; i++) {
                  ids.push(tabs[i].id);
    ...
    snap: function () {
      ...
      const [_height, _width] = [
        screen.availHeight,
        _snap === "s_left" ? 0 : parseInt(screen.availWidth / 2),
      ];
    ...
    set_bk: function () {
      let theFunction = function () {
        chrome.browserSettings.openBookmarksInNewTabs
          .get({})
    ...
    set_search: function () {
      let theFunction = function () {
        chrome.browserSettings.openSearchResultsInNewTabs
          .get({})
          .then(function (result) {
            if (result) {
              console.log(result);
    ...
    homepage: async function () {
      var theFunction = async function () {
        var _appname = "homepage";
        await sub.initAppconf(_appname);
        // 如果Chrome还支持topSites API，那么你可以像这样使用它
        chrome.topSites.get(async function (sites) {
          let _obj = {};
          _obj.sites = sites;
          _obj.listId = await new Promise((resolve, reject) => {
          ...
          sub.cons[_appname] = _obj;
          sub.insertTest(_appname); // 这个可能需要转成异步操作
        });
      };
      var thepers = ["topSites"];
      var theorgs;
    ...
    recentbk: function (call_appslist) {
      var theFunction = function () {
        ...
        chrome.bookmarks.getRecent(
          parseInt(config.apps[_appname].n_num),
          function (bkArray) {
            _obj.bk = bkArray;
            sub.cons[_appname] = _obj;
            sub.insertTest(_appname);
        ...
        chrome.management.getAll(function (exts) {
          ...
          for (var i = 0; i < exts.length; i++) {
            ...
            (function (i) {
              ...
              getBase64(
                exts[i].icons
                  ? exts[i].icons[exts[i].icons.length - 1].url
                  : "",
                exts[i].icons
                  ? exts[i].icons[exts[i].icons.length - 1].size
                  : 32,
                function (data) {
  ...
  open: function (url, target, position, pin, flag) {
    ...
    if (theTarget == "s_current") {
      ...
      var _obj =
        browserType == "fx" ? { url: theURL } : { url: theURL, focused: true };
  ...
  saveConf: async function (noInit, sendResponse) {
    ...
    if (_isSync) {
      chrome.storage.sync.clear(async function () {
        chrome.storage.sync.set(config, async function () {
          if (chrome.runtime.lastError) {
            ...
            await chrome.storage.sync.set(sub.cons.lastConf, function () {
              loadConfig(); //可能需要转成异步
              chrome.runtime.sendMessage({
                type: "confErr",
      ...
      chrome.storage.local.get(async function (items) {
        let _obj = {};
        _obj.config = config;
  ...
  upgrade: {
    ...
    _43: function () {
      ...
      for (var i = 0; i < _type.length; i++) {
        for (var ii = 0; ii < _array.length; ii++) {
          if (
            config[_type[i]].ui[_array[ii]] &&
            config[_type[i]].ui[_array[ii]].opacity
          ) {
            config[_type[i]].ui[_array[ii]].opacity =
              config[_type[i]].ui[_array[ii]].opacity * 100;
          }
        }
      }
    ...
    _36: function () {
      if (config.apps) {
        if (config.apps.rss) {
          config.apps.rss.n_pin =
            config.apps.rss.n_pin == "s_unpin" ? false : true;
        }
        if (config.apps.recentht) {
          config.apps.recentht.n_pin =
            config.apps.recentht.n_pin == "s_unpin" ? false : true;
        }
        if (config.apps.recentbk) {
          config.apps.recentbk.n_pin =
            config.apps.recentbk.n_pin == "s_unpin" ? false : true;
        }
      }
    ...
    _34: async function () {
      ...
      chrome.storage.local.get(async function (items) {
        if (items & items.apps) {
          ...
          delete items.apps;
          await chrome.storage.local.set({ items: items }); // 保存更改
        }
      });
      ...
      config.version = 34;
      await sub.saveConf(true); // 这个可能需要转成异步操作
    },
    ...
    _32: function () {
      ...
      for (var i = 0; i < acname0.length; i++) {
        for (var ii = 0; ii < acname1.length; ii++) {
          for (
            var iii = 0;
            config[acname0[i]][acname1[ii]] &&
            iii < config[acname0[i]][acname1[ii]].length;
            iii++
          ) {
      ...
      if (!config.apps) {
        ...
        if (config.apps.rss) {
          config.apps.rss.n_pin =
            config.apps.rss.n_pin == "s_pin" ? true : false;
        }
        if (config.apps.recentht) {
          config.apps.recentht.n_pin =
            config.apps.recentht.n_pin == "s_pin" ? true : false;
        }
        if (config.apps.recentbk) {
          config.apps.recentbk.n_pin =
            config.apps.recentbk.n_pin == "s_pin" ? true : false;
        }
      }
      config.version = 32;
      sub.saveConf(true);
    ...
    _31: function () {
      ...
      for (var i = 0; i < _drgtype0.length; i++) {
        for (var ii = 0; ii < _drgtype.length; ii++) {
          for (
            var iii = 0;
            config[_drgtype0[i]][_drgtype[ii]] &&
            iii < config[_drgtype0[i]][_drgtype[ii]].length;
            iii++
          ) {
            if (
              _erraction.contains(
                config[_drgtype0[i]][_drgtype[ii]][iii].name
              ) &&
              _des.contains(
              ...
              for (
                var j = 0;
                j < config[_drgtype0[i]][_drgtype[ii]][iii].selects.length;
                j++
              ) {
                if (
                  config[_drgtype0[i]][_drgtype[ii]][iii].selects[j].type ==
                  "n_optype"
                ) {
                  config[_drgtype0[i]][_drgtype[ii]][iii].selects[j].value =
                    "s_back";
                }
              }
            }
      ...
      for (var i = 0; i < config.mges.mges.length; i++) {
        if (config.mges.mges[i].name == "newtab") {
          for (
            ...
            ii < config.mges.mges[i].selects.length;
            ii++
          ) {
            if (
    ...
    f1to1_1: function () {
      ...
      for (var i = 0; i < config.mges.mges.length; i++) {
        ...
        // }
        if (
          config.mges.mges[i].name == "txtsearch" ||
          config.mges.mges[i].name == "imgsearch"
        ) {
        ...
        for (
          ...
          if (config.mges.mges[i].selects[ii].type.indexOf("s_") == 0) {
            config.mges.mges[i].selects[ii].type = config.mges.mges[i].selects[
              ii
            ].type.replace("s_", "n_");
          }
          if (
            ...
            config.mges.mges[i].selects[ii].type =
              "n_" + config.mges.mges[i].selects[ii].type;
          }
        }
      }
      ...
      for (var i = 0; i < 3; i++) {
        for (var ii = 0; ii < config.drg[arraydrg[i]].length; ii++) {
          ...
          for (
            var iii = 0;
            config.drg[arraydrg[i]][ii].selects &&
            iii < config.drg[arraydrg[i]][ii].selects.length;
            iii++
          ) {
            if (
              config.drg[arraydrg[i]][ii].selects[iii].type.indexOf("s_") == 0
            ) {
              config.drg[arraydrg[i]][ii].selects[iii].type = config.drg[
                arraydrg[i]
              ][ii].selects[iii].type.replace("s_", "n_");
            }
            if (
      ...
      for (var i = 0; i < 3; i++) {
        for (var ii = 0; ii < config.sdrg[arraydrg[i]].length; ii++) {
          ...
          for (
            var iii = 0;
            config.sdrg[arraydrg[i]][ii].selects &&
            iii < config.sdrg[arraydrg[i]][ii].selects.length;
            iii++
          ) {
            if (
              config.sdrg[arraydrg[i]][ii].selects[iii].type.indexOf("s_") == 0
            ) {
              config.sdrg[arraydrg[i]][ii].selects[iii].type = config.sdrg[
                arraydrg[i]
              ][ii].selects[iii].type.replace("s_", "n_");
            }
            if (
    ...
    f1_3to1_4: function () {
      ...
      for (var i = 0; i < config.mges.mges.length; i++) {
        if (
          ["up", "down", "top", "bottom"].contains(config.mges.mges[i].name)
        ) {
          config.mges.mges[i].selects.push({
            type: "n_scroll",
    ...
    f1_6to2_0: function () {
      ...
      if (
        config.general.engine.txtengine[0].url ==
    ...
    f2_0to2_2: function () {
      for (var i = 0; i < config.mges.mges.length; i++) {
        ...
        if (
          config.mges.mges[i].name == "recentbk" ||
          config.mges.mges[i].name == "recentht"
        ) {
          config.mges.mges[i].selects = [];
          config.mges.mges[i].checks = [];
          config.mges.mges[i].selects = [
    ...
    f2_5to2_6: function () {
      for (var i = 0; i < config.general.engine.txtengine.length; i++) {
        if (
          config.general.engine.txtengine[i].url ==
          "https://image.baidu.com/i?objurl=%s"
        ) {
          config.general.engine.txtengine[i].url =
    ...
    f2_8to2_9: function () {
      ...
      config.general.settings.boxzoom
        ? delete config.general.settings.boxzoom
        : null;
      var _array = [
        "n_reload",
        "n_stop",
      ...
      for (var i = 0; i < _arrayobj.length; i++) {
        for (var ii = 0; ii < _arrayobj[i].length; ii++) {
          for (
            var iii = 0;
            _arrayobj[i][ii].selects && iii < _arrayobj[i][ii].selects.length;
            iii++
          ) {
            if (_array.contains(_arrayobj[i][ii].selects[iii].type)) {
              _arrayobj[i][ii].selects[iii].type =
                _obj[_arrayobj[i][ii].selects[iii].type];
            }
          }
        }
      }
      ...
      for (var i = 0; i < config.general.script.script.length; i++) {
        config.general.script.script[i].content =
          config.general.script.script[i].script;
      ...
      for (var i = 0; i < config.general.engine.txtengine.length; i++) {
        config.general.engine.txtengine[i].content =
          config.general.engine.txtengine[i].url;
      }
      for (var i = 0; i < config.general.engine.txtengine.length; i++) {
      ...
      for (var i = 0; i < config.general.engine.imgengine.length; i++) {
        config.general.engine.imgengine[i].content =
          config.general.engine.imgengine[i].url;
      }
      for (var i = 0; i < config.general.engine.imgengine.length; i++) {
  ...
  funOnMessage: function (message, sender, sendResponse) {
    // console.log(message);
    // console.log(sender);
    // console.log(sendResponse.toString());
    sub.message = message;
    ...
    switch (message.type) {
      ...
      case "action_pop":
        ...
        /*theConf=config.pop.actions[_index]*/ sub.theConf =
          config.pop.actions[_index];
  ...
  apps: {
    autoreload: {
      reload: function (message, sender, sendResponse) {
        if (message.value.type == "start") {
          ...
          sub.cons.autoreload[sender.tab.id].iconCountdown =
            message.value.iconCountdown;
          sub.cons.autoreload[sender.tab.id].bypassCache =
            message.value.bypassCache;
    ...
    tablist: {
      ...
      tabSwitch: function (message) {
        message.value
          ? chrome.tabs.update(Number(message.value), { active: true })
          : null;
      },
    },
    jslist: {
          ...
          case "disable":
            if (
              ...
              (config.apps.extmgm.always &&
                config.apps.extmgm.always.contains(message.value.id))
            ) {
              break;
            }
          ...
          case "disableall":
            for (var i = 0; i < sub.cons.extmgm.ext_enabled.length; i++) {
              if (
                ...
                (config.apps.extmgm.always &&
                  config.apps.extmgm.always.contains(
                    sub.cons.extmgm.ext_enabled[i].id
                  ))
              ) {
                continue;
              }
        ...
        chrome.management.getAll(function (exts) {
          for (var i = 0; i < exts.length; i++) {
            if (
              exts[i].id == sub.extID ||
              (config.apps.extmgm.always &&
                config.apps.extmgm.always.contains(exts[i].id))
            ) {
              continue;
            }
    ...
    pxmovie: {
      ...
      getData: function (message, sender, sendResponse) {
        console.log("getData", message.value);
        // fetch(message.value)
        // 	.then(response=>response.blob())
        // 	.then(blob=>{
  ...
  IDB: {
    ...
    DBInit: function (
      db,
      storeName /*string*/,
      keyPath /*string*/ /*,indexarray,[{xx:"xx"},...]*/
    ) {
      return new Promise((resolve, reject) => {
...
if (!chrome.runtime.onInstalled) {
  ...
  chrome.runtime.onInstalled.addListener(function (details) {
    console.log("chrome.runtime.onInstalled.addListener", details.reason);
    // chrome.windows.getAll({ populate: true }, function (windows) {
    //   for (var i = 0; i < windows.length; i++) {
    ...
    switch (details.reason) {
      case "update":
        console.log(111);
        if (config && config.general && config.general.engine) {
...
if (chrome.browserSettings && chrome.browserSettings.contextMenuShowEvent) {
  ...
  chrome.storage.local.get("flag_mouseup", function (data) {
    if (data.flag_mouseup == null) {
